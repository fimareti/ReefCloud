...
```{r}
#| label: readData
dat <- read_csv("../data/Samoa_Waitt_Exhibition.csv", trim_ws = TRUE)
```

```{r}
#| label: readLabelset
labelset <- read_csv("../data/exported-labels-978.csv", trim_ws = TRUE) 
```
...
```{r}
dat <- dat |> 
  mutate(NorthSouth = ifelse(site_name %in% c(paste0("UPO_", c(20, 21, 22, 23, 25))), "North", "South"),
...
dat |>
  ...
  ggplot(aes(y =  100*COUNT/TOTAL, x = survey_start_date, colour = NorthSouth)) +
  geom_point() +
...
dat |>
  filter(type == "point_machine_classification", GROUP == "HC") |> 
  ggplot(aes(y =  100*COUNT/TOTAL, x = NorthSouth)) +
  geom_boxplot() 
```
...
form <- bf(COUNT | trials(TOTAL)~ NorthSouth + (1|site_name)+ (1|transect_id),
...
```
```{r}
model1 |> bayes_R2(summary = FALSE, re_form = ~(1|site_name) + (1| median_hdci))
 # median_hdci()#|>
 #knitr::kable()
```
...
g1<-
  model1 |>
  emmeans(~cots, type = "response") |>
  as.data.frame() |>
  #gather_emmeans_draws() |>
  #mutate(fit = plogis(.value)) |>
  #summarise(median_hdci(fit)) |>
  ggplot(aes(y = prob, x = cots))+
